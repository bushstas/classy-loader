		...
		for (let i = 1; i < parts.length; i++) {
			...
			if (hasTilda) {
				...
				while (typeof currentTagName == 'number' && typeof prevParts[prevParts.length - prevPartTry] != 'undefined') {
					currentTagName = getComponentTagName(prevParts[prevParts.length - prevPartTry], false, currentTagName);
					prevPartTry++;
				}
				if (!currentTagName) {
					throw new Error('Parsing error: loader can\'t parse a prefix from component tag name "' + value + '"');
				}
			}
