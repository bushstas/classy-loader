...
const parseItem = (source) => {
	...
	if (parts.length > 1) {
		...
		for (let i = 1; i < parts.length; i++) {
			...
			let inTag = isInTag(prevPart);
			con(inTag);

		
				// let p = prevPart.split(/<(?=[a-zA-Z\/])/),
				// 	pl = p.length,
				// 	l = p[pl - 1],
				// 	inTag = isInTag(l);
				// if (l[0] == '/' || (/[;\+:,\.\?\(\)\[\]\*\\\/\&\|\!]/).test(l)) {
			...
			for (let j = ps.length - 1; j >= 0; j--) {
				...
				switch (p) {
					...
					case ';':
						...
						} else if (!inContext && !inTagContent) {
							return false;
						}
					break;

					case '}':
						if (!inContext || inContext == p) {
							inContext = p;
							c1++;
						}
					break;
...
const parse = (source) => {
	...
	// let tagStart = '__TAG_START__';
	// source = obfuscatStrings(source);
	// source = source.replace(/([\w\]])\s+</g, "$1<")
	// 			   .replace(/\breturn\s*</g, 'return ' + tagStart)
	// 			   .replace(/([\r\n,\[\{\(=:]\s*)</g, "$1" + tagStart);

	// let content = [];
	// let tags = [];
	// let ps = source.split(tagStart);
	// content.push(ps[0]);
	// let currentTag = '',
	// 	openTags = 0;
	// for (let index = 1; index < ps.length; index++) {
	// 	if (!ps[index].match(/^[\/_a-zA-Z]/)) {
	// 		if (openTags > 0) {
	// 			currentTag += ps[index];
	// 		} else {
	// 			content.push(ps[index]);
	// 		}
	// 		continue;
	// 	}
	// 	if (openTags > 0) {
	...
	// 		openTags++;
	// 	}
	// 	let pps = ps[index].split('>');
	// 	if (openTags == 0) {
	// 		if (pps.length > 1) {
	// 			if (pps[0].indexOf('{') > -1) {
	// 				let a1 = pps[0].replace(/[^\{]/g, ''),
	// 					a2 = pps[0].replace(/[^\}]/g, '');
	// 				if (a1.length != a2.length) {
	// 					con(pps[0]);
	// 				}
	// 			}
	// 			tags.push('<' + pps[0].replace(/[\r\n\t]/g, ' ').replace(/ {2,}/g, ' ') + '>');
	// 			pps[0] = '';
	// 			pps = clean(pps.join('>'));
	// 			content.push(pps);
	// 		} else {
	...
	// 				if (openTags > 0) {
	// 					currentTag += pps[i];
	// 					if (typeof pps[i + 1] == 'string') {
	// 						currentTag += '>';
	// 					}
	// 					openTags--;
	// 					if (openTags == 0) {
	// 						tags.push(currentTag.replace(/[\r\n\t]/g, '').replace(/ {2,}/g, ' '));
	// 						currentTag = '';
	// 					}
	// 				} else {
	// 					cnt.push(pps[i]);
	...
	// 	}
	// }
	// source = deobfuscateStrings(s);
	// let parts = source.split('__DELIM__');
	
	// parseMode = 'code';
	// source = '';
	// for (let part of parts) {
	// 	if ((new RegExp(getMinimalRegex())).test(part)) {
	// 		part = parseItem(part);
	// 	}
	// 	source += part;
	// 	parseMode = parseMode == 'code' ? 'tag' : 'code';
	// }
					...
					if (p.match(IMAGE_SHORTCUTS_REGEX)) {
						...
						if (prts[1]) {
							...
							if (!imageSources[srcIndex]) {
								throw new Error('ClassyLoader css parsing error: cant\'t find image source with index "' + srcIndex + '"');
							}
							ext = ext.replace(/\d+/g, ''),
							prts[0] = '';
